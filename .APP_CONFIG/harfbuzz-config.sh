...
if [ ! -f $app-$version.$arch ]; then  
  filedwnld="https://github.com/harfbuzz/harfbuzz/releases/download/$version/$app-$version.$arch"
...
if [ $? -eq 0 ]; then  
  ...
  if [ $? -eq 0 ]; then    
    ...
    if [ -x $sapp/configure ]; then    
      ./$sapp/configure $XORG_CONFIG --with-fontconfig=yes --with-freetype=yes && popd
    fi 
  fi
fi
